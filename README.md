...
| Model | Accuracy | Macro F1 | Glioma Recall |
|---------|----------|-----------|---------------|
| ResNet-18 (Frozen) | ~75% | ~74% | ~61% |
| ResNet-18 (Fine-tuned) | ~93% | ~93% | ~79% |
| ResNet-50 (Fine-tuned) | ~94% | ~94% | ~81% |
| Vision Transformer (ViT-B/16) | ~93% | ~93% | ~79% |
| ResNet-50 (Class-weighted) | ~94% | ~94% | ~82% |

### Key Observations

- Fine-tuning significantly outperforms frozen transfer learning.
- ResNet-50 achieves the strongest baseline performance.
- Vision Transformers perform competitively but do not outperform CNN-based models on this dataset.
...
Example output generated by the MRI reporting workflow:

```text
Prediction: No Tumor
Confidence Score: 0.9632
Confidence Level: High

Interpretability Notes:
The model predicts no tumor with high confidence.
Grad-CAM analysis indicates localized attention patterns.
...
```bash
python3 src/training/train_resnet18_frozen.py
```
...
```bash
python3 src/training/train_resnet50.py
```

Train ResNet-50 (Class-weighted):

```bash
python3 src/training/train_resnet50_weighted.py
```

Train Vision Transformer:

```bash
python3 src/training/train_vit.py
```

Run Grad-CAM analysis:

```bash
python3 src/analysis/gradcam_analysis.py
```
...
Generate an MRI analysis report:
